...
نسخهٔ ۰٫۴: حساب کاربری، شش ترجیح، مدیریت کارها، برنامه‌ریز روز/هفته و **ذخیرهٔ برنامه با تاریخچه و Undo/Redo** با FastAPI و PostgreSQL.
...
برنامه‌ریز کارها را در وقت آزاد می‌چیند، کلاس و قفل معتبر را حفظ می‌کند و کارهای جا نشده را با دلیل نشان می‌دهد. پیش‌نمایش را می‌توان در درخواست جدا ذخیره کرد، زمان کار را ویرایش کرد و تغییر را برگرداند یا دوباره اعمال کرد. رابط تقویم، تکرار خودکار فعالیت‌ها و اتصال AI هنوز ساخته نشده‌اند. وضعیت آزمون‌ها در [BUILD_STATUS](docs/BUILD_STATUS.md) ثبت شده است.
...
بررسی راه‌اندازی دوباره، برنامهٔ ذخیره‌شده، مسیر Undo/Redo و رسید درخواست تکراری را نیز پوشش می‌دهد. هنگام ارتقا به ۰٫۴، migration شمارهٔ ۳ لازم است؛ اجرای `python -m smartplanner.migrate` آن را بدون حذف دادهٔ قبلی اعمال می‌کند. قبل از ارتقای دادهٔ واقعی نسخهٔ پشتیبان بگیرید؛ بازگرداندن خودکار این migration ارائه نشده است.
